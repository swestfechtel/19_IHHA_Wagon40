...
\bigskip\noindent\TODO{
	...
	\begin{enumerate}
		...
		\item \sout{$p_{bc} \sim F_{b}$, ausgangs-}
...
\bigskip\noindent\TODO{
	...
	\begin{itemize}
		...
		\item \sout{$m \propto F_{b}$}
		\item \sout{$F_{B} \propto a$}
		\item \sout{$m \sim a$}
		\item \sout{$v \propto d$}
		\item \sout{$a \propto d$}
	\end{itemize}
}
...
\bigskip\noindent\TODO{\begin{itemize}
		\item \sout{Wheel Rail Friction}
		\item \sout{Track gradient}
\end{itemize}}
...
\bigskip\noindent\TODO{
	\sout{Konstanten fuer ${\mathcal{M}}$?}
}
...
\bigskip\noindent\TODO{
	$\rightarrow$ chapter 3
}

\section{Time considerations}
\label{sec:TimeConsiderations}
\par\noindent
So far, we have looked at the braking process from a discrete point of view. However, in reality it is a continuous process and we thus have to take time into consideration. 

\bigskip\noindent\TODO{
	Zeitlich diskret $\rightarrow$ zeitlich kontinuierlich
}